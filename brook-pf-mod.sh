...
Download_brook(){
    ...
    cd ${file}
    if [[ ${bit} == "x86_64" ]]; then
        wget --no-check-certificate -N "https://cdn.jsdelivr.net/gh/txthinking/brook@releases/download/${brook_new_ver}/brook"
    else
        wget --no-check-certificate -N "https://cdn.jsdelivr.net/gh/txthinking/brook@releases/download/${brook_new_ver}/brook_linux_386"
...
Service_brook(){
    if [[ ${release} = "centos" ]]; then
        ...
        if ! wget --no-check-certificate https://cdn.jsdelivr.net/gh/monret/brook@master/brook-pf_debian -O /etc/init.d/brook-pf; then
            echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
        fi
        chmod +x /etc/init.d/brook-pf
